...
This version does not support data transfer rates other than 5MS/s
while supporting the full range of IFREQs from 0 to 2.5MHz.
The reason is a data tranfer bottleneck due to USB transfer to the ADALM2000
and the need for 4 bytes/sample (32bit) for the ADALM2000.

Future versions may support higher band center frequencies 
in case an upmixing module is coupled to the ADALM2000, but this option
is yet to be implemented.

The oversampling rate is >= 15, thus allowing for a gain of resolution of
nearly 2 bits or more, depending on the upper corner frequency of the RF band.
...
class SDR_control(QObject):
    ...
    def identify(self):
        ...
        : rtype: dict
        """
        device_ID_dict = {"rates": {0:0, 2500000:1},
    ...
    def set_rec(self):
        self.modality = "rec"
        errorstate = True
        value = "cannot record, RX mode not yet available in this device"
