...
class playrec_worker(QObject):
    ...
    def __init__(self, stemlabcontrolinst,*args,**kwargs):
        ...
        self.stopix = False
        self.DATABLOCKSIZE_BASIC = 4096*16#4096*256*32
    ...
    def play_loop_filelist(self):
        ...
        ################TODO TODO TODO: calculate OSR correctly
        #OSR must be such that SRDAC/highestfreq = nearest integer
        #WRONG: OSR = int(np.floor(SRDAC/(np.floor(2 * (lo_shift+sampling_rate/2))))) #OSR is the floor integer value of SRDAC/fNyquist, while fNyquist = 2 * (lo_shift + sampling_rate/2)
        OSR = 30
        #OSR = 3
...
############################ we have in data what in ADALM prototype is chunk
                    ...
                    if not self.get_pause():
                        try:
                            ...
                            if formatstring == "s16le":
                                ...
                                #prototype: ffmpeg_process.stdin.write(chunk)
                                #prototype: ffmpeg_process.stdin.flush()
